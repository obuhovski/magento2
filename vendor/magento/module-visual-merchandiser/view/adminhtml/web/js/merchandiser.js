...
define([
    ...
    $.widget('mage.visualMerchandiser', {
        ...
        _dialogSave: function (event, selected, modal) {
            ...
            sortData.each(function (pair) {
                if (parseInt(pair.value, 10) >= maxPosition) {
                    maxPosition = parseInt(pair.value, 10) + 1;
                }
            });

            $(selected).each(function (idx, entityId) {
                if (sortData.get(entityId) === undefined) {
                    selectedSortData.set(entityId, maxPosition++);
                } else {
                    selectedSortData.set(entityId, sortData.get(entityId));
                }
            });
